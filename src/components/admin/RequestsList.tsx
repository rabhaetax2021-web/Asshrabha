...
export default function RequestsList({ initial, type }: { initial: any[]; type: 'deposit' | 'withdraw' }) {
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      {items.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>No requests.</p> : (
          ...
          {items.map(it => (
            <div key={it.id} style={{ display: 'flex', justifyContent: 'space-between', padding: 8, borderRadius: 6, background: 'var(--bg-secondary)' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {it.status === 'PENDING' && (
                  <>
                    <button className="btn btn-primary" onClick={() => doAction(it.id, 'approve')} disabled={processing === it.id}>{processing === it.id ? '...' : 'Approve'}</button>
                    <button className="btn btn-ghost" onClick={() => doAction(it.id, 'reject')} disabled={processing === it.id}>{processing === it.id ? '...' : 'Reject'}</button>
                  </>
                )}
