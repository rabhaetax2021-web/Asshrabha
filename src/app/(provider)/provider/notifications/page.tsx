...
export default async function ProviderNotificationsPage() {
  ...
  if (!current || !isProvider(current.role as any)) {
  ...
  return (
    <section className="provider-page container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        <div>
          <h1>Notifications</h1>
          <p style={{ color: 'var(--text-muted)', marginTop: 8 }}>
            Your provider notification history.
          </p>
        </div>
        <Link href="/provider" className="btn btn-ghost" style={{ whiteSpace: 'nowrap' }}>
          Back to dashboard
        </Link>
      </div>
