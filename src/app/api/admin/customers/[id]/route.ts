...
export async function POST(request: NextRequest, context: { params: Promise<Record<string, unknown>> }) {
  try {
    ...
    if (action === 'approve') {
      const result = await approveCustomer(id, adminUserId, note)
      return NextResponse.json({ ok: true, result })
    }
    if (action === 'reject') {
      const result = await rejectCustomer(id, adminUserId, note)
      return NextResponse.json({ ok: true, result })
    }
    if (action === 'suspend') {
      const result = await (await import('@/lib/actions/admin.actions')).suspendCustomer(id, adminUserId, note)
      return NextResponse.json({ ok: true, result })
    }
