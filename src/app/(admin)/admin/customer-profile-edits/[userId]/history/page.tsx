...
export default async function CustomerEditHistoryPage({ params }: { params: { userId: string } }) {
  const current = await getCurrentUser()
  if (!current || !['ROOT_ADMIN','SUB_ADMIN'].includes(current.role) || current.status !== 'APPROVED') return <div>Unauthorized</div>

  const userId = params.userId
  const edits = await prisma.customerProfileEdit.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, include: { requester: true } })

  return (
    <section className="admin container">
      <h1>Customer Edit History</h1>
        ...
        {edits.map(e => (
          <div key={e.id} style={{ border: '1px solid #e5e7eb', padding: 12, borderRadius: 8 }}>
            ...
            <div style={{ marginTop: 12, display: 'grid', gap: 8 }}>
              {buildCustomerEditChangeSummary(e.changes as Record<string, unknown> | null | undefined, undefined).map((item) => (
